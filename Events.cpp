...
void OnPlayerHurt(CPlayerrecord* _playerRecord, ShotResult &_ShotResult, int hitgroup, int damage)
{
	...
	{
		// set tick
		LastShot.m_iTickHitPlayer = g_ClientState->m_nServerTick;
...
void GameEvent_PlayerDeath_ProcessQueuedEvent(const GameEvent_PlayerDeath_t& gameEvent)
{
	...
	if (_playerRecord)
	{
		...
		_playerRecord->m_iLastKilledTickcount = g_ClientState->m_nServerTick;
	}
}
	...
	//if (_InflictorWasLocalPlayer)
	{
		...
		if (LocalPlayer.ShotRecord)
		{
			if (!LocalPlayer.ShotRecord->m_pInflictedEntity)
			{
				...
				{
					if (LocalPlayer.m_iLastAASwapTick != g_ClientState->m_nServerTick)
					{
						...
						if (fovy < ANTI_AA_YAW && fovx < ANTI_AA_PITCH)
						{
							...
							LocalPlayer.m_iLastAASwapTick = g_ClientState->m_nServerTick;
#endif
						}
					}
				}
			...
			else
			{
				...
				if (_playerRecord)
				{
					...
					if (LastShot.m_iTickHitWall != g_ClientState->m_nServerTick)
					{
						// update impact tick
						LastShot.m_iTickHitWall = g_ClientState->m_nServerTick;

						if (!((_Entity->GetAlive() || (_playerRecord->m_iLastKilledTickcount == g_ClientState->m_nServerTick /*&& LastShot.m_iTickHitWall != Interfaces::Globals->tickcount*/))))
						{
							int fucker = 0;
						}
				...
				else if (_InflictorWasLocalPlayer)
				{
					...
					if (fabsf(Interfaces::Globals->realtime - LocalPlayer.ManualFireShotInfo.m_flLastManualShotRealTime) < 1.0f)
					{
						...
						if (LocalPlayer.ManualFireShotInfo.m_iTickHitWall != g_ClientState->m_nServerTick)
						{
							// update impact tick
							LocalPlayer.ManualFireShotInfo.m_iTickHitWall = g_ClientState->m_nServerTick;

							// don't process the tick if we hit the player
							if (LocalPlayer.ManualFireShotInfo.m_iTickHitPlayer != g_ClientState->m_nServerTick)
							{
