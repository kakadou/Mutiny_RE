...
//get the number of ticks elapsed between two cycles, note: does not currently support looping animations
int CCSGOPlayerAnimState::BacktraceCycle(float flPreviousCycle, int layer, bool is_looping_NOT_SUPPORTED_FIXME)
{
	C_AnimationLayer* pLayer = pBaseEntity->GetAnimOverlay(layer);

	if (is_looping_NOT_SUPPORTED_FIXME)
		DebugBreak();

	CPlayerrecord *pCPlayer = pBaseEntity->ToPlayerRecord();
	C_AnimationLayer* pServerLayer = &pCPlayer->DataUpdateVars.m_PostAnimLayers[layer];

	int i = 0;

	if (m_bOnLadder || !pLayer || pServerLayer->_m_flPlaybackRate <= 0.0f)
		return i;

	float flTargetCycle = pServerLayer->_m_flCycle;

	if (flTargetCycle != flPreviousCycle)
	{
		//int totalcmds = pCPlayer->m_iTicksChoked + 1;

		float flCurrentCycle = flPreviousCycle;
		float flTimeDelta = m_flLastClientSideAnimationUpdateTimeDelta != 0.0f ? m_flLastClientSideAnimationUpdateTimeDelta : TICK_INTERVAL;
		while (flCurrentCycle < flTargetCycle)
		{
			flCurrentCycle += pServerLayer->_m_flPlaybackRate * flTimeDelta;
			++i;
		}
	}
	else
	{
#if defined INTERNAL_DEBUG || defined _DEBUG
		auto owner = pLayer->GetOwner();
		if (owner)
		{
			std::string name = adr_util::sanitize_name((char*)owner->GetName().data());
			printf("BacktraceCycle: new cycle on %s matched previouscycle! This shouldn't happen on layer 4!!\n", name.data());
		}
		else
		{
			printf("BacktraceCycle: new cycle matched previouscycle! This shouldn't happen on layer 4!!\n");
		}
#endif
	}
	return i;
