...
bool CBaseEntity::SetupBonesFix(matrix3x4_t* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime)
{
	...
	return GetFunction<Fn>(pRenderable, 13)(pRenderable, pBoneToWorldOut, nMaxBones, boneMask, currentTime);
}
...
bool CBaseEntity::SetupBonesRebuilt(matrix3x4_t* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime)
{
		...
		//commented
		//auto cache = SetupBonesFix(pBoneToWorldOut, nMaxBones, boneMask, currentTime);
...
BOOLEAN CBaseEntity::IsPlayer()
{
	...
	return client_class->m_ClassID == _CCSPlayer;
...
void C_CSGOPlayerAnimState::SetupVelocity()
{
	...
	if (bJustStartedMovingLastUpdate && m_flFeetWeight <= 0.0f)
	{
		...
		if (sequence != -1)
		{
			if (*(DWORD*)((DWORD)pBaseEntity->pSeqdesc(sequence) + 196) > 0)
			{
