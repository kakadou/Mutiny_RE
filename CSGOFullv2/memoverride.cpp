...
typedef struct localeinfo_struct2
{
	pthreadlocinfo locinfo;
	pthreadmbcinfo mbcinfo;
} _locale_tstruct2, *_locale_t2;
...
__declspec(dllexport) void BuiltDebug() {}
#endif
...
{
	if (GetValveAllocator()->IsDebugHeap() )
	{
...
{
#if !defined(USE_LIGHT_MEM_DEBUG) && !defined(USE_MEM_DEBUG)
	return GetValveAllocator()->Realloc(pMem, nSize);
#else
	return GetValveAllocator()->Realloc(pMem, nSize, g_pszModule, 0);
#endif
}
...
inline void *AllocUnattributed( size_t nSize )
{
	return GetValveAllocator()->Alloc(nSize);
}
...
// end up in a recursion (as GetValveAllocator()->Alloc() calls malloc)
#if _MSC_VER >= 1400
#define ALLOC_CALL __declspec(noalias) _CRTRESTRICT 
#define FREE_CALL __declspec(noalias)
#else
#define ALLOC_CALL
#define FREE_CALL
#endif
...
void *_recalloc_base( void *pMem, size_t count, size_t size)
{
	void *pMemOut = ReallocUnattributed( pMem, size * count );
	memset(pMemOut, 0, size * count);
	return pMemOut;
}
...
{
	return GetValveAllocator()->GetSize(pMem);
}
...
int _heapchk()
{
	return GetValveAllocator()->heapchk();
}
...
{
	return GetValveAllocator()->Alloc(nSize, pFileName, nLine);
}

void free_db( void *pMem, const char *pFileName, int nLine )
{
	GetValveAllocator()->Free(pMem, pFileName, nLine);
}

void *realloc_db( void *pMem, size_t nSize, const char *pFileName, int nLine )
{
	return GetValveAllocator()->Realloc(pMem, nSize, pFileName, nLine);
}
...
void __cdecl operator delete( void *pMem )
{
	...
	GetValveAllocator()->Free(pMem);
#else
	GetValveAllocator()->Free(pMem, g_pszModule, 0);
#endif
}
...
public:
	CAttibCRT(int nBlockUse) : m_nBlockUse(nBlockUse)
	{
		...
		{
			GetValveAllocator()->PushAllocDbgInfo(CRT_INTERNAL_FILE_NAME, 0);
		}
	}
	...
	~CAttibCRT()
	{
		if (m_nBlockUse == _CRT_BLOCK)
		{
			GetValveAllocator()->PopAllocDbgInfo();
		}
	}
...
{
	AttribIfCrt();
	void *pMem = GetValveAllocator()->Alloc(nSize * nNum, pFileName, nLine);
...
{
	AttribIfCrt();
	return GetValveAllocator()->Realloc(pMem, nNewSize, pFileName, nLine);
}
...
{
	return GetValveAllocator()->CrtSetDbgFlag( nNewFlag );
}
...
{
	return GetValveAllocator()->CrtSetBreakAlloc( lNewBreakAlloc );
}
					 
int __cdecl _CrtIsValidHeapPointer( const void *pMem )
{
	return GetValveAllocator()->CrtIsValidHeapPointer( pMem );
}

int __cdecl _CrtIsValidPointer( const void *pMem, unsigned int size, int access )
{
	return GetValveAllocator()->CrtIsValidPointer( pMem, size, access );
}

int __cdecl _CrtCheckMemory( void )
{
	// FIXME: Remove this when we re-implement the heap
	return GetValveAllocator()->CrtCheckMemory( );
}
...
{
	// FIXME: Remove this when we re-implement the heap
	GetValveAllocator()->CrtMemCheckpoint( pState );
}
...
{
	return GetValveAllocator()->CrtSetReportMode( nReportType, nReportMode );
}

_HFILE __cdecl _CrtSetReportFile( int nRptType, _HFILE hFile )
{
	return (_HFILE)GetValveAllocator()->CrtSetReportFile( nRptType, hFile );
}

_CRT_REPORT_HOOK __cdecl _CrtSetReportHook( _CRT_REPORT_HOOK pfnNewHook )
{
	return (_CRT_REPORT_HOOK)GetValveAllocator()->CrtSetReportHook( pfnNewHook );
}
        ...
        int nLine, const char * szModule, const char * szFormat, ... )
{
	...
	return GetValveAllocator()->CrtDbgReport( nRptType, szFile, nLine, szModule, output );
}
...
{
	//DYLAN CHANGED 
		return AllocUnattributed(nSize);//_heap_alloc(nSize);
}
...
LPVOID WINAPI XMemAlloc( SIZE_T dwSize, DWORD dwAllocAttributes )
{
	...
	if ( !bPhysical && !pAttribs->dwHeapTracksAttributes && pAttribs->dwAllocatorId != eXALLOCAllocatorId_XUI )
	{
		...
		case XALLOC_ALIGNMENT_4:
			ptr = GetValveAllocator()->Alloc( dwSize );
			break;
...
VOID WINAPI XMemFree( PVOID pAddress, DWORD dwAllocAttributes )
{
	...
	if ( !bPhysical && !pAttribs->dwHeapTracksAttributes && pAttribs->dwAllocatorId != eXALLOCAllocatorId_XUI )
	{
		...
		{
		case XALLOC_ALIGNMENT_4:
			return GetValveAllocator()->Free( pAddress );
		default:
...
SIZE_T WINAPI XMemSize( PVOID pAddress, DWORD dwAllocAttributes )
{
	...
	if ( !bPhysical && !pAttribs->dwHeapTracksAttributes && pAttribs->dwAllocatorId != eXALLOCAllocatorId_XUI )
	{
		...
		{
		case XALLOC_ALIGNMENT_4:
			return GetValveAllocator()->GetSize( pAddress );
		default:
...
class _LocaleUpdate
{
    _locale_tstruct2 localeinfo;
    _ptiddata ptd;
    bool updated;
    public:
    _LocaleUpdate(_locale_t2 plocinfo)
        : updated(false)
    {
		/*
        if (plocinfo == NULL)
