...
IBaseClientDll* FixedClient = nullptr;
vmthook* client_hook;
using newCreateMove_t = void(__thiscall*)(IBaseClientDll*, int, float, bool);
void __stdcall new_hooked_createmove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket)
{
	static auto original_fn = client_hook->get_func_address <newCreateMove_t>(22);
	original_fn(FixedClient, sequence_number, input_sample_frametime, active);
	CUserCmd* m_pcmd = Interfaces::Input->GetUserCmdCMD(sequence_number);
	CVerifiedUserCmd* verified = Interfaces::Input->GetVerifiedUserCmd(sequence_number);
	...
	LocalPlayer.Entity = LocalPlayer.GetLocalPlayer();


	bSendPacket = CurrentUserCmd.bSendPacket;
...
void SetupVMTHooks()
{
	FixedClient = get_interface<IBaseClientDll>(("client.dll"), ("VClient0"));
	...
	while (!Interfaces::Globals)
		Interfaces::Globals = **(IGlobalVarsBase***)((*(DWORD**)Interfaces::Client)[0] + 0x1F);
	...
	Interfaces::Input = *reinterpret_cast<CInput**>(FindSignature(("client.dll"), "B9 ? ? ? ? F3 0F 11 04 24 FF 50 10") + 0x1);
	...
	client_hook = new vmthook(reinterpret_cast<DWORD**>(FixedClient));
	client_hook->hook_function(reinterpret_cast<uintptr_t>(hooked_createmove_naked), 22);
...
//#endif
	...
	oDrawModel = (DrawModelFn)StudioRenderHk->HookFunction((DWORD)Hooks::DrawModel, 29); 
	oPaintTraverse = (PaintTraverseFn)VPanel->HookFunction((DWORD)Hooks::PaintTraverse, 41);
	oFrameStageNotify = (FrameStageNotifyFn)Client->HookFunction((DWORD)Hooks::FrameStageNotify, 37);
	...
	oIsHLTV = (IsHLTVFn)Engine->HookFunction((DWORD)Hooks::IsHLTV, 93); 
